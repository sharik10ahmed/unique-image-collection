...
export const Route = createFileRoute("/admin/content")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Edit the hero banner, promo strip and highlight copy shown on the storefront." },
    ...
    <AdminLayout title="Homepage & Content" subtitle="Edit the copy shown across the storefront">
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-sm border border-border bg-card p-6">
          ...
          <div className="space-y-4">
            <Field label="Heading">
              <Input value={content.promo.heading} onChange={(e) => setContent({ ...content, promo: { ...content.promo, heading: e.target.value } })} />
            </Field>
            <Field label="Message">
              <Textarea rows={3} value={content.promo.text} onChange={(e) => setContent({ ...content, promo: { ...content.promo, text: e.target.value } })} />
            </Field>
          </div>
          ...
          <div className="space-y-4">
            {content.why.map((w) => (
              <div key={w.id} className="space-y-2 border-b border-border pb-4 last:border-0">
                <Input
                  value={w.title}
                  onChange={(e) => setContent({ ...content, why: content.why.map((x) => (x.id === w.id ? { ...x, title: e.target.value } : x)) })}
                />
                <Textarea
                  rows={2}
                  value={w.text}
                  onChange={(e) => setContent({ ...content, why: content.why.map((x) => (x.id === w.id ? { ...x, text: e.target.value } : x)) })}
                />
              </div>
            ))}
          </div>
