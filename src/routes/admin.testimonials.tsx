...
export const Route = createFileRoute("/admin/testimonials")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Add and moderate customer testimonials shown on the storefront." },
...
function AdminTestimonials() {
  ...
  const save = () => {
    ...
    setTestimonials((prev) =>
      editing.id ? prev.map((t) => (t.id === editing.id ? editing : t)) : [...prev, { ...editing, id: "t" + (Date.now() % 100000) }],
    );
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title={editing?.id ? "Edit Testimonial" : "Add Testimonial"}>
        {editing ? (
          <div className="space-y-4">
            <Field label="Customer name"><Input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} /></Field>
            <Field label="Location"><Input value={editing.location} onChange={(e) => setEditing({ ...editing, location: e.target.value })} /></Field>
            <Field label="Message"><Textarea rows={4} value={editing.message} onChange={(e) => setEditing({ ...editing, message: e.target.value })} /></Field>
