  ...
  return (
    <div className="flex min-h-screen bg-muted/60">
      ...
      >
        ...
        <p className="mt-1 text-[10px] uppercase tracking-[0.24em] text-sidebar-primary">Admin Panel</p>
        ...
        <div className="mt-8 border-t border-sidebar-border pt-5">
          <Link to="/" className="block px-3 py-2 text-xs text-sidebar-foreground/70 hover:text-sidebar-primary">
            ← View storefront
          </Link>
          <button
            onClick={() => {
      ...
      {open ? <div className="fixed inset-0 z-50 bg-foreground/40 lg:hidden" onClick={() => setOpen(false)} /> : null}
...
export function DashboardCard({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    ...
    <div className="overflow-x-auto rounded-sm border border-border bg-card">
      <table className="w-full min-w-[720px] text-left text-sm">
        <thead className="border-b border-border bg-muted/60">
          <tr>
            {head.map((h) => (
              <th key={h} className="whitespace-nowrap px-4 py-3 text-[11px] uppercase tracking-[0.16em] text-muted-foreground">
                {h}
              </th>
            ))}
          </tr>
...
export function AdminActions({ onEdit, onDelete }: { onEdit?: () => void; onDelete?: () => void }) {
  return (
    <div className="flex gap-2">
      {onEdit ? <Button size="sm" variant="outline" onClick={onEdit}>Edit</Button> : null}
      {onDelete ? <Button size="sm" variant="ghost" onClick={onDelete}>Delete</Button> : null}
    </div>
  );
}